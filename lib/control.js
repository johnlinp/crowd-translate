...
var setApiPaths = function(app) {
    ...
    app.get('/api/translation/list/mine', function(request, response) {
        models.Author.find({ user: request.user })
                .populate({ path: 'translation', populate: { path: 'original' } })
                .exec(function(err, authors) {
            if (err) throw err;

            response.json(authors.map(function(author) {
                return author.translation;
            }));
        });
    });
