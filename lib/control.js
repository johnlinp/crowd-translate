...
var setApiPaths = function(app) {
    var apiError = function(request, response, errMsg) {
        response.json({
            success: false,
            errMsg: errMsg,
        });
    ...
    var updateTranslation = function(request, response, updater) {
        if (!request.user) {
            apiError(request, response, 'not authorized');
            return;
        }

        var translationId = request.body.translationId;

        if (!translationId) {
            apiError(request, response, 'translation id not provided');
            return;
        }

        models.Translation.findOne({ _id: translationId })
                .populate('author')
                .exec(function(err, translation) {
            if (err) {
                apiError(request, response, 'database query error');
                return;
            }

            if (!translation) {
                apiError(request, response, 'translation not found');
                return;
            }

            if (translation.author.id != request.user.id) {
                apiError(request, response, 'invalid author');
                return;
            }

            updater(translation, request);
            translation.save(function(err) {
                if (err) {
                    apiError(request, response, 'database save error');
                    return;
                }
    ...
    app.post('/api/translation/save/', function(request, response) {
        updateTranslation(request, response, function(translation, request) {
            translation.title = request.body.title || '';
            translation.texts = request.body.texts || [];
        });
    });

    app.post('/api/translation/publish/', function(request, response) {
        updateTranslation(request, response, function(translation, request) {
